...
export const init = (self, v) => {
  ...
  if (stopUpdateDistance < 0) globalThis["stopUpdateDistance"] = Infinity;
  if (halfSample) {
    uiAlphaFactor = 0.5;
    uiAnimeSpeed = 2;
    force[0] *= 2;
  ...
  for (const offset of endPosOffsets) lasers.push({
...
export const onEvents = (self, _ref) => {
  ...
  if (bake) {
    ...
    if (OnStartLevel || OnPlayerDeadEnd) lasers.forEach(_ref2 => {
      let {
        laser
      } = _ref2;
      return laser.clearRays();
    });
    if (OnPhysicsUpdate) {
      if (levelManager.timerEnabled && math.distanceFloat3(player.position, self.getTransform()[0]) > stopUpdateDistance) {
        for (const {
          laser
        } of lasers) if (!laser.frozen) laser.freeze();
      } else {
        for (const {
          laser
        } of lasers) if (laser.frozen) laser.unfreeze();
      }
      if (halfSample && (sample = !sample)) return;
      const selfPos = self.getTransform()[0];
      const selfRot = self.getRotationQuaternion();
      for (const {
        vector,
        ...
        laser.updatePlayerStates(damageTable[player.ballType] ?? damageTable.Default, heatFactor > 0 ? heatFactor * ((600 - player.temperature) / 300) : heatFactor * ((player.temperature + 120) / 80), chargeFactor, player.temperature > -20 ? dryFactor : 0, uiAlphaFactor, uiAnimeSpeed);
      }
    }
  }
};