...
export const init = (self, v) => {
  ...
  lasers.push({
    ...
    laser: new Laser(
      material,
      self.getComponent("Settings").getData("Tags"),
      !bake,
      !bake
    )
  });
...
{let { OnPhysicsUpdate, OnPlayerDeadEnd, OnStartLevel } = _ref;
  if (bake) {
    if (OnStartLevel || OnPlayerDeadEnd) {
      ...
      for (const { vector, maxDistance, laser } of lasers) {
        ...
        laser.updateRays(
          selfPos,
          mathEx.transFloat3WithQuat(vector, selfRot),
          maxDistance,
          thickness
        );
        laser.freeze();
    ...
    if (OnStartLevel || OnPlayerDeadEnd)
    lasers.forEach((_ref2) => {let { laser } = _ref2;return laser.clearRays();});
    if (OnPhysicsUpdate) {
      if (
      ...
      stopUpdateDistance)
      {
        ...
        if (!laser.frozen) laser.freeze();
      } else {
        for (const { laser } of lasers)
        if (laser.frozen) laser.unfreeze();
      }
      ...
      for (const { vector, maxDistance, laser } of lasers) {
        ...
        laser.updatePlayerStates(
          damageTable[player.ballType] ?? damageTable.Default,
          heatFactor > 0 ?
          heatFactor * ((600 - player.temperature) / 300) :
          heatFactor * ((player.temperature + 120) / 80),
          chargeFactor,
          player.temperature > -20 ? dryFactor : 0,
          uiAlphaFactor,
          uiAnimeSpeed
        );
      }
