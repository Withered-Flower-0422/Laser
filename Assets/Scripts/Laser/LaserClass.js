...
export class Ray {
    ...
    enable(startPos, endPos, castItem, thickness) {
        ...
        if (scene.getItem(this.rayItem.guid))
            this.rayItem.setTransform(...calRayTransform(startPos, endPos, thickness));
    }
    disable() {
        this._enabled = false;
        if (scene.getItem(this.rayItem.guid))
            this.rayItem.setScale(zeroFloat3);
    }
    destroy() {
...
export class Laser {
    ...
    static isCasted(item, ...excludeLasers) {
        return this.instances.some(l => !excludeLasers.includes(l) && l.isCasted(item));
    }
    static countCasted(item, ...excludeLasers) {
    ...
    updateRays(startPos, vector, maxDis, thickness) {
        ...
        for (let j = i; j < raysData.length; j++) {
            ...
            if (ray)
                ray.enable(startPos, endPos, castItem, thickness);
            else
                this.rays[j] = new Ray(startPos, endPos, castItem, thickness, this.material);
        }
    }
    applyForce(linearKp, angularKp) {
        ...
        for (let i = 0; i < this.rays.length; i++) {
            ...
            if (!enabled || !castItem)
                break;
            if (!isAffectedByLaserForce(castItem))
                continue;
            const physicsObject = castItem.getComponent("PhysicsObject");
            if (physicsObject?.getData("PhysicsBodyType") !== 2)
                continue;
            const { linear, angular } = mathEx.getVelocityByForceAtPoint(castItem.getTransform()[0], castItem.getRotationQuaternion(), endPos, math.normalizeFloat3(mathEx.subFloat3(endPos, startPos)), physicsObject.getLinearVelocity(), physicsObject.getAngularVelocity(), physicsObject.getMass(), linearKp, angularKp);
    ...
    updatePlayerStates(damage, heat, charge, dry, uiAlphaFactor = 1, uiAnimeSpeed = 1) {
        ...
        if (castCnt > 0 && damage > 0) {
            ...
            if (this.hurtUI.alpha > 0)
                this.hurtUI.alpha -= speed;
        }
    }
    isCasted(item) {
