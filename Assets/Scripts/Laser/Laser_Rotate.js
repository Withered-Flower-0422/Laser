...
{let { OnLoadLevel, OnTimerActive, OnPhysicsUpdate, OnPlayerDeadEnd, OnStartLevel } = _ref;
  if (OnLoadLevel) {
  ...
  if (OnPhysicsUpdate && start) {
    laser.setRotationQuaternion(
      mathEx.mulQuaternion(
        initialRotQuat,
        math.float3ToQuaternion(
          new Float3(0, angle = (angle + rotateSpeed) % 360, 0)
        )
      )
    );
  }
