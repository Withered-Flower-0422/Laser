...
const explode = (self) => {
  ...
  for (const items of Object.values(scene.sphereCastAll(selfPos, range))) {
    for (const item of items) {
      ...
      const disFactor = Math.max(
        math.distanceFloat3(selfPos, itemPos) ** 0.5,
        1
      );
      physicsObject.setLinearVelocity(
        mathEx.addFloat3(
          physicsObject.getLinearVelocity(),
          mathEx.scaleFloat3(
            math.normalizeFloat3(
              mathEx.subFloat3(itemPos, selfPos)
            ),
            explodeForce / disFactor / physicsObject.getMass()
          )
        )
      );
...
{let { OnPlayerDeadEnd, OnStartLevel, OnPhysicsUpdate } = _ref2;
