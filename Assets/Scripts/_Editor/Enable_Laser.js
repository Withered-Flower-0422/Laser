...
const addHurtUITexAsset = () => {
  ...
  const assetReference = scene.getAllObjects().find(obj => obj.type === "Settings").getComponent("AssetReference");
  const data = JSON.parse(assetReference.getData());
  if (data.Textures.includes(path)) return true;
...
const createRayTemplate = () => {
  ...
  for (const obj of scene.getAllObjects()) {
    ...
    if (JSON.parse(obj.getComponent("Settings").getData()).TemplateName === "LaserRay") res.push(obj);
  }
  if (res.length === 0) {
...
export const execute = () => {
  if (!addHurtUITexAsset()) {
    dialogWindowManager.openMessageDialog(menuPath, editor.language === "Chinese" ? '"Screen_Red.tex" 材质文件不存在，请将其添加到 "Assets/Textures/Screen/" 文件夹中。' : '"Screen_Red.tex" not found. Please add it to "Assets/Textures/Screen/" folder.', editor.language === "Chinese" ? "确认" : "Ok", () => {});
    return;
  }
  const rays = createRayTemplate();
  if (rays.length > 1) {
    dialogWindowManager.openMessageDialog(menuPath, editor.language === "Chinese" ? "检测到场景中存在多个激光模板，请删除多余的模板。" : "Detected more than one LaserRay template in the scene. Please remove the extra ones.", editor.language === "Chinese" ? "确认" : "Ok", () => {});
  } else {
    dialogWindowManager.openMessageDialog(menuPath, editor.language === "Chinese" ? "激光已启用。" : "Laser has been enabled.", editor.language === "Chinese" ? "确认" : "Ok", () => {});
  }
};